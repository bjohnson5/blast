...
Phase I of BLAST development is scheduled for January - April of this year and is focused on creating lightweight models of common LN node implementations. I are about 3 months into the proposed 4 month phase and have made some great progress researching how LND uses memory. In this context I are defining `model` as a software application that emulates how a real Lightning Node operates, but it has been optimized for a simulation environment. By optimizing these models I hope to be able to run large test networks with many nodes and channels. Because LND is written in `Go` and `Go` has good profiling tools built into it, and because LND is a widely used implementation, I decided to start there.
...
I developed a Docker container equipped with essential memory profiling and resource monitoring tools, alongside the LND and Bitcoin Core software. Additionally, the container includes a block explorer and Python for performance analysis and graphical representation of both lnd and Bitcoin Core.
...
Using `pprof`, I can see what functions allocate the most memory and which ones could potentially be altered to use less memory in a simulation environment. The general approach of BLAST is to find parts of LN node implementations that are not important in a testing use case and tweak those parts to use less resources so that large test networks can be created. Once I had all the tools in place and `lnd` setup correctly I executed the following steps:
 1. Run an `lnd` instance in our Docker container 
...
After setting up a way to run multiple `lnd` instances in `goroutines` I repeated our profiling process on 100 `lnd` nodes, each running in its own `goroutine`.
...
If you have a network of Lightning Nodes you can run `sim-ln` on that network and it will generate payments between nodes and log the results of those payments. It can operate in a random mode that just generates general network traffic or in a defined mode where the user can specify what payments are made and how often. BLAST will eventually allow a user to define and start up a large test Lightning Network and perform operations on that network. BLAST will need a payment generator to create "noise" on the network and to let the user create different test scenarios. I believe that BLAST and `sim-ln` will work perfectly together, so I decided to start getting involved in its development alongside developing BLAST.

I opened Issue [#168](https://github.com/bitcoin-dev-project/sim-ln/issues/168) and began implementing it. From the issue description:
...
After completing the implementation of this feature and doing the proper testing. I opened PR [#173](https://github.com/bitcoin-dev-project/sim-ln/pull/173) and worked with the other contributors to review and clean up the feature. This PR has now been merged into `sim-ln`.
...
- Perform our profiling process on the completed LND model and compare the results to the original benchmarks.
